...
// updates function for NotGate and generates an event based on the input state
Event* NotGate::update(uint64_t current_time)
{
	// initializes the default state
	char state = '0';
	Event* e = nullptr;
	// determines the output state based on the input state
	...
	// if the state has changed, create a new event.
	if(state != m_current_state)
	{
		m_current_state = state;
		uint64_t next = current_time + m_delay;
