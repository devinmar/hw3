        ...
        if(line == "GATES")
        {
            ...
            for(int i = 0;i<n;i++)
            {
                ...
                //Add code here to support the NOT gate type
								if(s_type == "NOT")
                {
									// reading and parsing the input wire ID for the NOT gate
                    std::string s_in1;
                    getline(ss, s_in1, ',');
										// reading and parsing the output wire ID for the NOT gate
                    std::string s_output;
                    getline(ss, s_output, ',');
										// creates a new instance of NotGate using the parsed input and output wire IDs
										//  stoi function converts the string IDs to integers
										// new gate is then added to the m_gates vector for later processing
